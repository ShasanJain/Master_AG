...
export default function JournalistCommandCenter() {
  ...
  const [media, setMedia] = useState('ltx2');
  ...
  const handleExecute = () => {
    ...
    const sse = new EventSource(`/api/journalist?topic=${encodeURIComponent(topic)}&model=${encodeURIComponent(model)}&media=${encodeURIComponent(media)}&status=${encodeURIComponent(status)}`);
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 rounded-xl bg-[var(--background)] border border-[var(--border)]">
                    <label className="text-[9px] font-black uppercase tracking-widest text-[var(--faint)] mb-1 block">Logic Engine</label>
                    <select
                      value={model}
                      onChange={(e) => setModel(e.target.value)}
                      disabled={isRunning}
                      className="w-full bg-[var(--surface)] text-xs font-mono text-amber-500 border border-[var(--border)] rounded px-2 py-1 outline-none cursor-pointer"
                    >
                      <option value="llama3.2">Llama 3.2 (Local)</option>
                      <option value="hermes3">Hermes 3 (Local)</option>
                    </select>
                  </div>
                  <div className="p-4 rounded-xl bg-[var(--background)] border border-[var(--border)]">
                    <label className="text-[9px] font-black uppercase tracking-widest text-[var(--faint)] mb-1 block">Media Engine</label>
                    <select
                      value={media}
                      onChange={(e) => setMedia(e.target.value)}
                      disabled={isRunning}
                      className="w-full bg-[var(--surface)] text-xs font-mono text-purple-400 border border-[var(--border)] rounded px-2 py-1 outline-none cursor-pointer"
                    >
                      <option value="ltx2">Modal LTX-2</option>
                ...
                <div className="p-4 rounded-xl bg-[var(--background)] border border-[var(--border)]">
                  <label className="text-[9px] font-black uppercase tracking-widest text-[var(--faint)] mb-1 block">CMS Status</label>
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => setStatus('published')}
                      disabled={isRunning}
                      className={`flex-1 py-1.5 border rounded text-[10px] font-mono font-bold tracking-widest uppercase transition-colors ${
                        status === 'published' 
