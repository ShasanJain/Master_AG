...
def generate_text_content(topic, model="llama3.2"):
...
def generate_video(topic, media="ltx2"):
    if media == "none":
        print("[+] Skipping media generation as requested.")
        sys.stdout.flush()
        return None
        
    fallback_video = "https://cdn.coverr.co/videos/coverr-abstract-neon-lights-5136/1080p.mp4"
    if media == "stock":
        print("[+] Injecting stock B-Roll video...")
        sys.stdout.flush()
...
def publish_to_ghost(title, html_content, status="published"):
    print(f"[+] Broadcasting to Ghost CMS with status '{status}'...")
    sys.stdout.flush()
...
def run_pipeline(topic, model="llama3.2", media="ltx2", status="published"):
    print(f"=== Starting Autonomous Pipeline for: '{topic}' ===")
    print(f"[*] Configuration - Model: {model} | Media: {media} | Status: {status}")
    sys.stdout.flush()
    
    # 1. Text Generation & Title Extraction
    title, html = generate_text_content(topic, model)
    
    # 2. Media Generation
    video_path = generate_video(topic, media)
    if video_path:
        video_html = f'''
<!--kg-card-begin: html-->
...
'''
    ...
    # 3. Distribution
    post_url = publish_to_ghost(title, html, status)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Autonomous Journalist Pipeline")
    parser.add_argument("topic", help="Target topic for the article")
    parser.add_argument("--model", default="llama3.2", help="Ollama model to use")
    parser.add_argument("--media", default="ltx2", choices=["ltx2", "stock", "none"], help="Media generation mode")
    parser.add_argument("--status", default="published", choices=["published", "draft"], help="Ghost CMS publish status")
    
    args = parser.parse_args()
    run_pipeline(args.topic, args.model, args.media, args.status)
