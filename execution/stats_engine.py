...
def get_stats():
    ...
    # 5. Database Health Memory Node Count from openmemory.db
    memory_nodes = 0
    db_path_om = os.path.join(BASE_DIR, "openmemory.db")
    if os.path.exists(db_path_om):
        try:
            conn = sqlite3.connect(db_path_om)
            cur = conn.cursor()
            cur.execute("SELECT COUNT(*) FROM memories")
            memory_nodes = cur.fetchone()[0]
            conn.close()
        except:
            pass
